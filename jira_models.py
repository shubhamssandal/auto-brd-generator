...
# --- JIRA-010: one representation for every proposed requirement change --------

# Where the evidence for a proposed change came from. The first four are the
# ingestion routes ``NormalizedTranscript.source`` already uses, so a meeting change
# carries the same source name the transcript arrived under; ``jira`` is a direct edit
# made to an issue this app created. The detector treats all five identically -- only
# the label shown to the reviewer differs.
CHANGE_SOURCES = ("manual", "upload", "google_meet", "microsoft_teams", "jira")

_SOURCE_LABELS = {
    "manual": "Pasted meeting notes",
    "upload": "Uploaded transcript",
    "google_meet": "Google Meet transcript",
    "microsoft_teams": "Microsoft Teams transcript",
    "jira": "Jira issue edit",
}

# The four change kinds JIRA-010 asks for, plus ``UNCLEAR`` for evidence that does
# not support any of them. ``UNCLEAR`` is not a fifth category of change: it is the
# refusal to classify one, and nothing in it can be applied.
CHANGE_TYPES = ("NEW", "CHANGED", "REMOVED_DEFERRED", "UNCHANGED", "UNCLEAR")

# Change kinds that would alter the BRD if a reviewer approved them. ``UNCHANGED``
# and ``UNCLEAR`` are deliberately absent: there is nothing to apply.
APPLICABLE_CHANGE_TYPES = ("NEW", "CHANGED", "REMOVED_DEFERRED")

# ``pending`` is the only state a freshly detected change may hold, whatever the
# model said. ``jira_only`` records "the Jira wording stands, the BRD does not
# change" -- a decision, not a mutation.
APPROVAL_STATES = ("pending", "approved", "rejected", "jira_only")


@dataclass(frozen=True)
class RequirementChange:
    """
    One proposed reconciliation between the approved BRD and newer state.

    Both entry paths produce this same shape: a meeting brought evidence that a
    requirement should change, or an issue in Jira was edited away from what the BRD
    says. Keeping one model is the point of JIRA-010 -- the review UI, the approval
    gate and the apply step then have a single kind of thing to handle.

    A change is a *proposal*. Nothing here has been applied to the BRD and nothing
    here has been written to Jira. ``old_text``, ``affected_plan_keys`` and
    ``affected_issue_keys`` are always filled in from stored state rather than from a
    model response, so a change cannot claim a requirement said something it did not,
    or name work it is not actually linked to.
    """
    ...
    requirement_id: str = ""
    old_text: str = ""
    proposed_new_text: str = ""
    source_evidence: str = ""
    # Which meeting or which issue the evidence came from, for the reviewer's benefit.
    source_reference: str = ""
    # Jira-driven changes only: which field drifted, what it currently holds, and what
    # this app last synchronized into it. ``previous_value`` is what makes the drift
    # reviewable -- without it the reviewer cannot tell an edit from the original.
    jira_issue_key: str = ""
    jira_field: str = ""
    jira_current_value: str = ""
    previous_value: str = ""
    # Filled in from the stored plan and creation results, never from the model.
    affected_plan_keys: tuple = ()
    affected_issue_keys: tuple = ()
    impact: str = ""
    proposed_action: str = ""
    # The model's own explanation, kept apart from ``impact`` so the reviewer can see
    # which sentence came from stored mappings and which came from a language model.
    rationale: str = ""
    confidence: str = ""
    # Set when the evidence does not support a classification, or when validation
    # could not confirm something a decision would depend on. Such a change is shown
    # and explained, but cannot be approved.
    ...
    def source_label(self) -> str:
        return _SOURCE_LABELS.get(self.source_type, self.source_type or "unknown source")

    @property
    def is_from_jira(self) -> bool:
        return self.source_type == "jira"

    @property
    def is_decidable(self) -> bool:
        """Whether a reviewer may approve this change at all."""
        if self.needs_manual_review or self.change_type not in APPLICABLE_CHANGE_TYPES:
            return False
        if self.change_type == "NEW":
            return bool(self.requirement_id and self.proposed_new_text)
        if self.change_type == "CHANGED":
            return bool(self.requirement_id and self.old_text and self.proposed_new_text)
        return bool(self.requirement_id and self.old_text)

    @property
    def mutates_brd(self) -> bool:
        """Whether applying the reviewer's decisions should change the BRD."""
        return self.approval_state == "approved" and self.is_decidable
...
class ChangeProposal:
    """
    Everything one detection run produced: the proposals, and what it could not do.

    ``notes`` carries the honest remainder -- evidence that named an unknown
    requirement, a Jira issue with no requirement behind it, a model response that
    could not be read. Those are reported rather than dropped silently, because a
    reconciliation that quietly ignored half its input would be worse than one that
    found nothing.
    """
    ...
    def pending(self) -> tuple:
        return tuple(change for change in self.changes if change.is_pending)

    @property
    def decidable(self) -> tuple:
        return tuple(change for change in self.changes if change.is_decidable)

    @property
    def needing_review(self) -> tuple:
        return tuple(change for change in self.changes if change.needs_manual_review)

    @property
    def approved(self) -> tuple:
        return tuple(change for change in self.changes if change.mutates_brd)
